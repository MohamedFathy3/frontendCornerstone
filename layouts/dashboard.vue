                ...
                <div class="grow">
                    <ul class="space-y-3 text-sm">
                        <template v-for="item in menu" :key="item.path">
                            <li>
                                <NuxtLink :to="item.subMenus.length > 0 ? '' : item.path" :class="[$route.path !== item.path ? 'text-white hover:bg-white/15' : 'gap-2 bg-white text-slate-800', 'flex items-center p-1.5 rounded-md group']">
                                ...
                                <ul v-if="item.subMenus.length > 0" class="bg-white/10 rounded-xl p-2 mt-2 gap-2">
                                    <li v-for="subItem in item.subMenus" :key="subItem.path" class="py-1.5">
                                        <NuxtLink :to="subItem.path" :class="[$route.path !== subItem.path ? 'text-white hover:bg-white/15' : 'gap-2 bg-white text-slate-800', 'flex items-center p-1.5 rounded-md']">
                ...
                <div class="border-t pt-3 border-white/10 text-white">
                    <div v-if="userStore.user" class="flex items-center gap-3">
                        <div class="w-12 h-12 ring-4 ring-white/10 rounded-full">
                            <NuxtImg v-if="userStore.user.imageUrl" class="w-12 h-12 object-cover bg-white !rounded-full" :src="userStore.user.imageUrl" :alt="userStore.user.name" :title="userStore.user.name" />
                        </div>
                        <div>
